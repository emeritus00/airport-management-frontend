...
function AircraftList() {
  ...
  const fetchAircrafts = async () => {
    try {
      const response = await axios.get("http://app-alb-918023355.ca-central-1.elb.amazonaws.com/aircrafts");
  ...
  const handleAdd = async () => {
    try {
      await axios.post("http://app-alb-918023355.ca-central-1.elb.amazonaws.com/aircraft", newAircraft);
      fetchAircrafts();
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(
        `http://app-alb-918023355.ca-central-1.elb.amazonaws.com/aircrafts/${editAircraft.id}`,
        editAircraft
      );
      fetchAircrafts();
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://app-alb-918023355.ca-central-1.elb.amazonaws.com/aircrafts/${id}`);
      fetchAircrafts();
    } catch (error) {
